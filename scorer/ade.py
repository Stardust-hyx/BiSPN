    ...
    for interaction in sent['interactions']:
        ...
        for a_obj in ta['names'].values():
            for b_obj in tb['names'].values():
                ...
                rel = {
                    'head': list(ta['names'])[0], 'head_type': ta['label'],
                    'tail': list(tb['names'])[0], 'tail_type': tb['label'],
                    'type': interaction['label']
                }
                relations.append(rel)
    return relations

def evaluate_ade(truth_sentences, pred_sentences, keys):
    # Evaluate NER
    ...
    for key in keys:
        ...
        typed_pred = pred_sentences[key]['entities']
        # Update gt_entities
        batch_gt_entities = []
    ...
    for key in keys:
        ...
        typed_pred = pred_sentences[key]['interactions']
    ...
    # Macro scores
    m_score = entity_score['ALL']
    relex_all_score = relation_score['ALL']
    print('Entity Precison = {}, Recall = {}, F1 = {}'.format(m_score['Macro_p'], m_score['Macro_r'], m_score['Macro_f1']))
    print('Triple Precison = {}, Recall = {}, F1 = {}'.format(relex_all_score['Macro_p'], relex_all_score['Macro_r'], relex_all_score['Macro_f1']))
    return {'f1': (m_score['Macro_f1'] + relex_all_score['Macro_f1']) / 2}
